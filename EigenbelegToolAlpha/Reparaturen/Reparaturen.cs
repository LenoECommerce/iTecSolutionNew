...
namespace EigenbelegToolAlpha
{
    public partial class Reparaturen : Form
    {
        ...
        public static bool IsEntryCreated {  get; set; }
        ...
        public void WorkWithSpecificRepKPITracking()
        {
            using (var form = new WorkWithSpecificRep())
            {
                // new logic: only work with internal number as most important id
                var result = form.ShowDialog();
                if (result == DialogResult.OK)
                {
                    int rowIndex = -1;
                    foreach (DataGridViewRow row in reparaturenDGV.Rows)
                    {
                        int rowIntern = Convert.ToInt32(row.Cells[1].Value);
                        if (rowIntern == form.internFiltered)
                        {
                            rowIndex = row.Index;
                        }
                    }
        ...
        public void WorkWithSpecificRep()
        {
            using (var form = new WorkWithSpecificRep())
            {
                // new logic: only work with internal number as most important id
