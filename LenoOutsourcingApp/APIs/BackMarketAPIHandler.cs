...
namespace EigenbelegToolAlpha
{
    public class BackMarketAPIHandler
    {
        ...
        public static string userAgent = "Aufermann & Dange Online Handel GbR";
        ...
        public string[] PullBuyBackDataFromOrderID_OldCode(string inputShippingNumber)
        {
            try
            {
                string[] feedback = new string[8];
                string response = GetRequest("https://www.backmarket.fr/ws/buyback/v1/orders?page=1&pageSize=100&status=RECEIVED");
                JObject json = JObject.Parse(response);
                JArray results = (JArray)json["results"];
                foreach (JObject result in results)
                {
                ...
                if (string.IsNullOrEmpty(feedback[0]))
                {
